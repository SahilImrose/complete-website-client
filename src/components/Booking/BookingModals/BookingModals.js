...
const BookingModal = ({ closeModal, modalIsOpen, service, date }) => {
  const handleClick = () => {
  ...
  const handlePayment = paymentId => {
    const orderDetails = {services: service.servicesName,shipment: servicesData, paymentId, orderTime: new Date() };
  ...
  return (
    <div>
      ...
      >
        <h2 className="text-center text-brand">{service.servicesName}</h2>
        ...
        <div id="payment"><ProcessPayment handlePayment={handlePayment}></ProcessPayment></div>
      </Modal>
    </div>
  );
};
